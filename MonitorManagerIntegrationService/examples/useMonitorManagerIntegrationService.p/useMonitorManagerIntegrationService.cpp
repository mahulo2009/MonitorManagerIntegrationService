...
// Includes
#include "useMonitorManagerIntegrationService.h"

#ifdef VXWORKS
int useMonitorManagerIntegrationService(char *devicename)
{
	if(devicename==NULL)
	{
		cerr << "Usage:  useMonitorManagerIntegrationService(devicename)" << endl;
		return(1);
	} // Default
#else
int main(int argc, char** argv)
{
	...
	char *devicename = cl.getOption("-name","Test/MonitorManagerIntegrationService_1");
#endif
	CORBA::Object* obj = 0;
	MONITORMANAGERINTEGRATIONSERVICE::MonitorManagerIntegrationService_ifce* device = 0;
	int quit=FALSE;
	int option, result=0;
	...
	try
	{
		...
		try
		{
			device = MONITORMANAGERINTEGRATIONSERVICE::MonitorManagerIntegrationService_ifce::_narrow(obj);
		}
		...
		{
			cerr << "Reference is not a MONITORMANAGERINTEGRATIONSERVICE::MonitorManagerIntegrationService_ifce" << endl;
			throw 0;
		}
		printf(" - ping MonitorManagerIntegrationService\n");
		device->ping();
		printf(" - device found\n");
